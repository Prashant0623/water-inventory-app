...
public interface WaterRepo extends JpaRepository<WaterModel, Long> {
	...
	@Query(value = "SELECT* FROM waterinventory.water_model where vendor_id=:id", nativeQuery = true)
	public List<WaterModel> findByVendorId(Long id);

	@Query(value = "SELECT* FROM  waterinventory.water_model where date=:date", nativeQuery = true)
	public List<WaterModel> findByDate(String date);

	@Query(value = "SELECT* FROM  waterinventory.water_model where date=:date AND vendor_id=:id", nativeQuery = true)
	public List<WaterModel> findByDateAndVendorId(String date, Long id);

	@Query(value = "SELECT * FROM waterinventory.water_model f order by f.id desc ", nativeQuery = true)
